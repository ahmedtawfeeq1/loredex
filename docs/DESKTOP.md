...
2. Because it's unsigned, Gatekeeper says the app **"is damaged."** It isn't — it's an unsigned download. On Apple Silicon, clearing the quarantine alone isn't enough; the app also needs a valid signature, so run **both** in Terminal:
   ```sh
   xattr -cr /Applications/Loredex.app
   codesign --force --deep --sign - /Applications/Loredex.app
   open /Applications/Loredex.app
   ```
   (`xattr -cr` clears quarantine; `codesign --sign -` ad-hoc-signs it, which is the part `xattr` alone misses.) No Terminal? Right-click the app → **Open** → **Open**.
3. Launch normally from Applications thereafter.
